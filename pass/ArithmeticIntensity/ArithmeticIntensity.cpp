...
// Arithmetic Intensity Analysis Driver
...
//    - if/else: max AI of all branch
// 3) Annotate the function with the arithmetic intensity (bandwidth and compute)
//    - add a new attribute to the function arguments
//
// Symbolic equations are built as `AffineExpr`s over a per-function table of
// "leaf" values (function block arguments, `tt.get_program_id`,
// `tt.get_num_programs`, and any unknown integer producers we encounter).
// The structured representation gives us free constant folding and
// canonicalisation; at serialisation time the expression is simplified and
// converted to a string by replacing each symbol token (`s0`, `s1`, ...)
// with its source-level name and rewriting the AffineExpr printer keywords
// (`floordiv`, `mod`) to a more conventional `/` / `%`.
...
public:
  ...
  // keywords (`floordiv`, `ceildiv`, `mod`) to `/` and `%`.
